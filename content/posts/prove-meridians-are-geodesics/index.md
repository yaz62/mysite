...
$$
The definition is essentially saying that geodesics are parallel transport of its tangent vectors. The geometrical relationship among relevant quantities are summarized in the following diagram:

<p style="text-align: center;">
    <img src="diagram.png" alt="Diagram" style="width:60%;" />
</p>

Then the recipe of the proof is quite straightforward:
- A parametrization of the surface of revolution $x(u, v)$;
- Calculate $x_u$ and $x_v$ as bases of the tangent plane;
- A parametrization of the meridian $\alpha$ on the surface, as function of $u$ and/or $v$;
...
The second to last step may sound simple, but usually involves Christoffel symbols which can be a quite a hassle. Instead, I decided to change the last two steps a bit to make the calculations easier:
...
So, the first step is parametrize an arbitrary surface of revolution as:
$$
...
a normal component, which is to say that $\lVert D_{\alpha^\prime(u)} \alpha^\prime(u)\rVert = D_{\alpha^\prime(u)} \alpha^\prime(u)\cdot n$. Both sides can be easily computed from above results:
$$
\begin{align\*}
...
What's more amazing to me is the role of LLMs. ChatGPT 4o did mention arclength parametrization at first, but it didn't show this is the key to the puzzle, and instead gave me some hand waving explanations. Claude failed to provide the answer too, and got the same confusion as I did, but still played the role of a "math buddy" exploring the problem with me. It noticed the algebraic connection of the equation to constant speed, which eventually became our "eureka!" moment. This experience of collaborative exploration brought more Dopamine than the answer itself. Of course, if I used ChatGPT o4-mini at the first place, I may save a few hours, but I think it will be way less fun.